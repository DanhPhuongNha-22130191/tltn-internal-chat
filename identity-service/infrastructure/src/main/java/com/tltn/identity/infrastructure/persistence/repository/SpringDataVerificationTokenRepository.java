package com.tltn.identity.infrastructure.persistence.repository;

import com.tltn.identity.infrastructure.persistence.entity.VerificationTokenJpaEntity;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;
import java.util.Optional;
import java.util.UUID;

@Repository
public interface SpringDataVerificationTokenRepository extends JpaRepository<VerificationTokenJpaEntity, UUID> {
    Optional<VerificationTokenJpaEntity> findByToken(String token);
    List<VerificationTokenJpaEntity> findByAccountIdAndTokenType(UUID accountId, String tokenType);
}
